...
class Board:
    ...
    def final_state(self, show = False):
        ...
        for col in range (COLS):
            ...
            if self.squares[0][col] == self.squares[1][col] == self.squares[2][col] != 0:
                if show:
                    ...
                    colour = circle_colour if self.squares[0][col] == w else cross_colour
                    #initial position
        ...
        #horizontal wins
        #need to loop through all rows.
        for row in range (ROWS):
            #this checks if each figure in a row is the same. It can not = 0 because it has to be a marked square. If that is not added in then it will also check empty rows as well. Which would not result in a win.
            if self.squares[row][0] == self.squares[row][1] == self.squares[row][2] != 0:
                #we want it to return anyone of the squares because they are all the same number. I just chose the first one. 
                #eg. 2 = 2 = 2 != 0, we return 2 when player two wins.
        ...
        #positve win
        if self.squares[2][2] == self.squares[1][1] == self.squares[0][0] != 0:
            return self.squares[0][0]
